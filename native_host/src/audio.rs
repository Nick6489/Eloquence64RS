...
impl AudioProcessor {
    ...
    pub fn new(sample_rate: u32) -> Self {
        Self {
            ...
            shelf: Biquad::new(high_shelf(sample_rate as f32, 2_500.0, 5.0)),
            body: Biquad::new(peaking_eq(sample_rate as f32, 1_400.0, 0.8, 0.4)),
            sample_rate,
    ...
    pub fn process(&mut self, input: &[i16]) -> Vec<i16> {
        ...
        for &sample in input {
            ...
            if !resample {
                output.push(Self::finish(shaped));
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn contour_keeps_its_acoustic_shape_at_both_rates() {
        for rate in [11_025, 16_000] {
            let low = processed_rms(rate, 250.0);
            let presence = processed_rms(rate, 4_000.0);
            let lift = 20.0 * (presence / low).log10();
            assert!(
                (3.5..6.0).contains(&lift),
                "{rate} Hz lift was {lift:.2} dB"
            );
        }
    }
